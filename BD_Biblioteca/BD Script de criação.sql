...
CREATE TABLE IF NOT EXISTS `Bibliotecadb`.`Categoria` (
  ...
  UNIQUE INDEX `CodCategoria_UNIQUE` (`CodCategoria` ASC) INVISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Bibliotecadb`.`Professores` (
  `MatSiape` INT NOT NULL AUTO_INCREMENT,
  `Nome` VARCHAR(45) NULL,
  `Endereco` VARCHAR(45) NULL,
...
DELETE FROM Usuario WHERE CodUsuario = NEW.Usuario_CodUsuario;
END IF;
END$$
